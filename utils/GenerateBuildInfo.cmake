...
function(_gbinfo_write_context_json target_name context_file)
    ...
    # Use PROJECT_SOURCE_DIR so when DChannel is built as subproject, source_dir points to DChannel, not parent
    if(target_name STREQUAL "${PROJECT_NAME}" AND DEFINED PROJECT_SOURCE_DIR)
        _json_str("${PROJECT_SOURCE_DIR}" _j_src_dir)
    else()
    ...
    # For multi-config generators (e.g. Visual Studio) CMAKE_BUILD_TYPE is empty; use DCHANNEL_BUILD_TYPE if set
    set(_gbinfo_build_type "${CMAKE_BUILD_TYPE}")
    if(_gbinfo_build_type STREQUAL "" AND DEFINED DCHANNEL_BUILD_TYPE AND NOT DCHANNEL_BUILD_TYPE STREQUAL "")
        set(_gbinfo_build_type "${DCHANNEL_BUILD_TYPE}")
    endif()
    _json_str("${_gbinfo_build_type}" _j_build_type)
